...
void main(void) {
	...
	while (1) {
		...
		// Tasteri i displej
		if (statusPrvogIspisa == 1) {
			setWritePointerToFirstChar();
			printChar(odabrano_K + '0'); // ispis karaktera, ascii tablea
		}
		statusPrvogIspisa = 0;

		Uint16 vrednostTastera;
		Uint16 vrednostTasteraPre = NoKey;
		while ((vrednostTastera = EZDSP5535_SAR_getKey()) == SW1) {
			if (vrednostTastera == SW1
					&& vrednostTastera != vrednostTasteraPre) {
				odabrano_K = (odabrano_K + 1) % 4;
				setWritePointerToFirstChar();
				printChar(odabrano_K + '0');
			}
			vrednostTasteraPre = vrednostTastera;
		}

		while ((vrednostTastera = EZDSP5535_SAR_getKey()) == SW2) {
			if (vrednostTastera == SW2
					&& vrednostTastera != vrednostTasteraPre) {
